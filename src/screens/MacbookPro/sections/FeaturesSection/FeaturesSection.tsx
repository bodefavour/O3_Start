      ...
      <div className="relative mx-auto flex w-full max-w-4xl flex-col items-center gap-6 px-4 text-center sm:px-6">
        ...
        <div className="mt-6 grid w-full gap-4 sm:grid-cols-3 sm:gap-6">
          {statistics.map((stat) => (
            ...
            >
              <span className="text-2xl font-extrabold text-[#00c48c] sm:text-3xl">{stat.value}</span>
              <span className="text-xs font-semibold text-white/90 sm:text-sm">{stat.label}</span>
            </div>
          ))}
        </div>
      </div>
