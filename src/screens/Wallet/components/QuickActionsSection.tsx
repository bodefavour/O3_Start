import { ArrowLeftRight, ArrowUpRight, Download } from "lucide-react";
import { Button } from "../../../components/ui/button";

interface QuickActionsSectionProps {
  onSendMoney: () => void;
  onReceiveMoney?: () => void;
  onSwapCurrencies?: () => void;
}

export const QuickActionsSection = ({
  onSendMoney,
  onReceiveMoney,
  onSwapCurrencies,
}: QuickActionsSectionProps) => {
  return (
    <div className="mt-8 mb-6">
      <h3 className="[font-family:'Inter',Helvetica] font-semibold text-[#0b1f3a] text-lg mb-4">
        Quick Actions
      </h3>
      <div className="grid gap-3 sm:grid-cols-3 sm:gap-4">
        <Button
          onClick={onSendMoney}
          className="h-[56px] w-full rounded-xl bg-[#00c48c] hover:bg-[#00b37d]"
        >
          <div className="flex items-center gap-2">
            <ArrowUpRight className="w-5 h-5 text-[#0b1f3a]" />
            <span className="[font-family:'Inter',Helvetica] font-semibold text-[#0b1f3a] text-base">
              Send Money
            </span>
          </div>
        </Button>

        <Button
          variant="outline"
          onClick={onReceiveMoney}
          className="h-[56px] w-full rounded-xl border-[#e5e7eb] bg-white hover:bg-gray-50"
        >
          <div className="flex items-center gap-2">
            <Download className="w-5 h-5 text-[#0b1f3a]" />
            <span className="[font-family:'Inter',Helvetica] font-semibold text-[#0b1f3a] text-base">
              Receive
            </span>
          </div>
        </Button>

        <Button
          variant="outline"
          onClick={onSwapCurrencies}
          className="h-[56px] w-full rounded-xl border-[#e5e7eb] bg-white hover:bg-gray-50"
        >
          <div className="flex items-center gap-2">
            <ArrowLeftRight className="w-5 h-5 text-[#0b1f3a]" />
            <span className="[font-family:'Inter',Helvetica] font-semibold text-[#0b1f3a] text-base">
              Swap
            </span>
          </div>
        </Button>
      </div>
    </div>
  );
};
